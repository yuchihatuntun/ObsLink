...
![](attachments/传送类指令一览表.png)
>[!danger] 核心规则
>1. 除了标志传送指令`SHAF`和堆栈操作指令`POPF`外，其余传送指令不影响状态标志。
>2. **语法规则**：
>    1. 立即数只能做源操作数 
>    2. 类型要一致
>    3. 类型要明确
...
`MOV` (Move) 其实有误导性。它并不是移动，而是**复制**。执行 `MOV AX, BX` 后，`BX` 的内容保持不变。

![](attachments/MOV%20指令适用范围.png)

### 取有效地址指令 `LEA`

其源操作数必须是**内存操作数**（即用`[]`括起来的那种地址，即偏移地址）。

`LEA` 的功能就是 ==**计算**== 源操作数（即有效地址），然后把这个**地址数值**传送到目的寄存器中，由于是直接写进目的寄存器里面，没有普通算术指令的写回周期，是==更加高效的加法运算指令==。
