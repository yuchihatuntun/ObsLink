...
要制备增强型（即要求 $V_{bi} > V_{P0}$），必须减小 $V_{P0}$。根据 $V_{P0} = \frac{ea^2 N_d}{2\epsilon_s}$，因此可以从两个工艺手段下手：

1. 薄的沟道厚度($a$)**。

2. 低沟道掺杂浓度($N_d$)**。

这样可以显著降低 $V_{P0}$，使其小于内建电势 $V_{bi}$，从而实现“常关”特性。
...
![](../attachments/沟道长度调制效应示意图.png)

当 $V_{DS}$ 继续增加超过饱和电压时，漏端的反向偏置电压进一步增大。这导致漏端附近的耗尽层不仅在垂直方向上夹断，还在**水平方向（沿沟道方向）** ==向源极延伸==。

**有效沟道长度 $L'$：**
$$
L' \approx L - \frac{\Delta L}{2}\quad \Delta L \approx \left[ \frac{2 \varepsilon_{s} ( V_{DS} - V_{DS(sat)} ) }{ e N_{d} } \right]^{1/2}
$$
根据电阻定律，沟道电阻与长度成正比。因此，电流与沟道长度成反比。

修正后的电流 $I'_{D1}$ 与原理想电流 $I_{D1}$ 的关系为：

$$
I'_{D1} = I_{D1} \times \frac{L}{L - \frac{\Delta L}{2}}
$$
为了方便计算，通常引入一个修正因子 $(1 + \lambda V_{DS})$，得到修正后的饱和电流方程：

$$
I'_{D1(sat)} = I_{D1(sat)} (1 + \lambda V_{DS})
$$
**输出阻抗（$r_{ds}$）：**
    $$r_{ds} = \frac{\partial V_{DS}}{\partial I'_{D1}} \approx \frac{\Delta V_{DS}}{\Delta I'_{D1}}$$
    
由于夹断后电流随电压略有增加（而非理想的零增加），JFET 在饱和区的输出阻抗 $r_{ds}$ 是**有限的**（而非理想的无穷大）。

### 速度饱和效应

![](../attachments/速度饱和效应.png)

当电场达到临界值 $E_{cn}$ 或 $E_{cp}$ 时，载流子速度达到极限值 $v_{sat}$（约为 $10^7 cm/s$ 数量级），此后不再随电场增加。

**发生条件：** 在沟道较短或偏置电压较高时，沟道漏端的电场可能在沟道尚未几何夹断之前，就已经大到使载流子速度达到 $v_{sat}$。

![](../attachments/速度饱和横截面.png)

$$
I_{D1}(sat) = e N_d v_{sat} (a - h_{sat}) W
$$
- $h_{sat}$：达到速度饱和时的耗尽层宽度。
