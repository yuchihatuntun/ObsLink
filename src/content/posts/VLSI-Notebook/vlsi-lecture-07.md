...
# <span style="color:rgb(107, 144, 222)">电路选择指南</span>

这章还是挺重要的，方便后续理解为什么选择各种各样的电路。

| 电路类型                                    | 特点与优势                                                                                                             | 缺点与局限                                                                                                          | 设计技巧/应用场景                                                                                                               |
| :-------------------------------------- | :---------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
| **静态 CMOS (Static CMOS)**               | 1. **鲁棒性强**：抗噪声能力强，无需电平恢复电路。<br>2. **无静态功耗**：理论上只有动态功耗。<br>3. **设计成熟**：有高度自动化的 EDA 工具和单元库支持。                      | **大扇入性能差**：大扇入逻辑门逻辑努力大，速度慢，需拆分为多级简单门。                                                                          | 1. **通用首选**：绝大多数电路的最佳选择。<br>2. **级联优化**：将复杂大扇入逻辑拆分为多级小扇入逻辑实现。                                                           |
| **伪 NMOS (Pseudo-NMOS)**                | 1. **面积小**：仅用 $N+1$ 个管子实现 $N$ 输入逻辑。<br>2. **输入电容小**：仅需驱动 NMOS，前级负载轻。<br>3. **适合大扇入**：消除了串联 PMOS 链，适合大扇入 NOR 门。    | 1. **静态功耗大**：输出低时存在 $V_{DD}$ 到地直流通路。<br>2. **有比电路**：$V_{OL} \neq 0$，噪声容限小，需精细调整尺寸比。                            | 1. **特定应用**：ROM, PLA, CAM 等大扇入阵列。<br>2. **功耗优化**：使用使能信号在闲置时关闭 PMOS 负载。<br>3. **尺寸设计**：通常设 PMOS 宽为 NMOS 的 $1/2$ 至 $1/6$。 |
| **传输管逻辑 (Pass Transistor / NMOS Only)** | 1. **晶体管极少**：实现相同逻辑所需管子数少（如 $N$ 个）。<br>2. **无静态功耗**：理想情况下无直流通路。<br>3. **小摆幅**：可利用 $V_{dd}-V_t$ 的小摆幅降低动态功耗（如位线）。   | 1. **阈值损失**：输出高电平仅为 $V_{dd}-V_t$。<br>2. **不可再生**：级联导致信号衰减，需插入反相器。<br>3. **后级漏电**：$V_{dd}-V_t$ 电平可能导致后级反相器静态功耗。 | 1. **延时优化**：后级反相器设计为 **Low-skewed**（降阈值）以加速响应。<br>2. **电平恢复**：使用 $V_T=0$ 管（需防漏电） 或电平恢复器。                                |
| **互补传输管逻辑 (CPL)**                       | 1. **逻辑丰富**：同构可实现 XOR, MUX 等多种逻辑。<br>2. **互补输出**：自带 $F$ 和 $\bar{F}$ 输出。<br>3. **速度快**：寄生电容小，结合 **SRPL** 恢复电路可获高速。 | 1. **阈值损失**：NMOS 传输存在“弱1”问题。<br>2. **布线复杂**：需要互补输入信号和双轨输出。                                                     | 1. **最佳传输管电路**：适合实现 XOR, MUX, 全加器。<br>2. **电平恢复**：使用交叉耦合 PMOS (SRPL) 加速翻转并恢复全摆幅。                                        |
| **CMOS 传输门 (Transmission Gate)**        | 1. **全摆幅**：NMOS+PMOS 并联，传输强0和强1。<br>2. **电阻恒定**：等效导通电阻在全电压范围内基本恒定。                                                | 1. **面积与电容大**：需 $2N$ 个管子，寄生电容较单管倍增。<br>2. **需互补时钟**：控制信号需 $C$ 和 $\bar{C}$。                                     | **通用模拟开关**：用于需要高线性度和全摆幅的信号传输场景。                                                                                         |
| **动态逻辑 (Dynamic Logic)**                | 1. **速度极快**：输入电容小（逻辑努力小），无短路电流。<br>2. **面积小**：$N+2$ 个管子。<br>3. **无毛刺**：单调跳变特性。                                    | 1. **信号完整性差**：易受电荷分享、时钟馈通、耦合噪声 影响。<br>2. **功耗高**：高开关活动性及大时钟负载。                                                 | 1. **高速处理器首选**：用于 ALU 关键路径。<br>2. **尺寸设计**：反相器设计为 **High-skewed**（大 $\beta$）以加速求值。<br>3. **必须仿真**：由于抗噪差，设计必须全面模拟验证。     |
